<script>
    ...
    // Subkategori diambil dari 15 data yang sedang tampil
    let availableSubcategories = $derived.by(() => {
        const subs = new Set();
        products.forEach(p => {
            if (p.subcategory && p.subcategory.trim() !== "") {
                subs.add(p.subcategory.charAt(0).toUpperCase() + p.subcategory.slice(1));
            }
        });
        ...
        {#if products.length > 0}
            ...
            <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4 text-[10px] md:text-xs text-gray-500 pb-4 border-b border-gray-50 w-full">
                ...
                <div class="flex items-center gap-1.5 min-w-0 overflow-hidden w-full lg:w-auto">
                    ...
                    {:else}
                        <!-- Total data sesungguhnya dari Database! -->
            ...
            <!-- Loop langsung menggunakan `products` -->
            ...
            <!-- Menggunakan currentPage & totalPages dari Backend -->
            {#if totalPages > 1}
