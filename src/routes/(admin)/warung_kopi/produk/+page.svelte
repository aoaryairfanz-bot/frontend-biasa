<script>
    ...
    // --- 4. FORM STATE MANUAL (UPDATED FOR 3 ZONES) ---
    let showModal = $state(false);
    let editingId = $state(null);
    let isUserEditingZones = $state(false); // Flag pengaman agar auto-fill tidak menimpa ketikan manual admin

    let formData = $state({
        name: '', sku: '', category: 'nonbook', subcategory: '', 
        price: '', price_zone_2: '', price_zone_3: '', // [+] Integrasi Kolom Multi-Zona
        strike_price: '', stock: '', description: '', discount_id: 0,
        weight: '', length: '', width: '', height: '', diameter: '', 
        isbn: '', publisher: '', author: '', publish_year: '', pages: '', book_version: '',
        foto_1_url: '', foto_2_url: '', foto_3_url: '' 
    });

    // [+] LOGIKA AUTO-FILL BERBASIS RUMUS MURNI (SVELTE 5 EFFECT RUNES)
    $effect(() => {
        const basePrice = Number(formData.price);
        if (basePrice && !editingId && !isUserEditingZones) {
            // Hanya aktif saat tambah produk baru dan admin belum mengubah zona manual
            formData.price_zone_2 = String(Math.floor(basePrice * 1.10)); // Murni +10% tanpa desimal
            formData.price_zone_3 = String(Math.floor(basePrice * 1.25)); // Murni +25% tanpa desimal
        }
    });
    ...
    function resetForm() {
        editingId = null; 
        isUserEditingZones = false;
        formData = { name: '', sku: '', category: 'nonbook', subcategory: '', price: '', price_zone_2: '', price_zone_3: '', strike_price: '', stock: '', description: '', discount_id: 0, weight: '', length: '', width: '', height: '', diameter: '', isbn: '', publisher: '', author: '', publish_year: '', pages: '', book_version: '', foto_1_url: '', foto_2_url: '', foto_3_url: '' };
    ...
    function openEditModal(product) {
        ...
        isUserEditingZones = true; // Kunci true saat edit agar data asli DB tidak ditimpa rumus otomatis
        formData = { 
            ...product, 
            name: product.name, 
            subcategory: product.subcategory, 
            discount_id: product.discount ? product.discount.id : 0, 
            price_zone_2: product.price_zone_2 !== undefined ? String(product.price_zone_2) : '', // Load data aman Zona 2
            price_zone_3: product.price_zone_3 !== undefined ? String(product.price_zone_3) : '', // Load data aman Zona 3
            foto_1_url: '', 
            foto_2_url: '', 
            foto_3_url: '' 
        };
    ...
    async function processBackgroundUpload(uploadId, dataPayload, filesPayload, editId) {
        ...
        try {
            ...
            // [+] Memasukkan 'price_zone_2' dan 'price_zone_3' ke jajaran kunci kirim data utama backend
            const keys = ['sku', 'category', 'price', 'price_zone_2', 'price_zone_3', 'stock', 'description', 'strike_price', 'weight', 'length', 'width', 'height', 'diameter', 'isbn', 'publisher', 'author', 'publish_year', 'pages', 'book_version'];
                ...
                <div class="md:col-span-8 space-y-5">
                    ...
                    <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm space-y-3">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                            <!-- [+] INTEGRASI MULTI-KOLOM GRID TACTICAL UNTUK 3 ZONA HARGA -->
                            <div>
                                <label class="block text-xs font-bold text-gray-600 mb-1">Harga Utama / Zona 1 <span class="text-red-500">*</span></label>
                                <input type="number" bind:value={formData.price} required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-100 outline-none text-sm font-bold text-gray-800" placeholder="0"/>
                                <span class="text-[9px] text-blue-600 font-bold mt-1 block">📍 Wilayah DIY & Jawa</span>
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-gray-600 mb-1">Harga Zona 2</label>
                                <input type="number" bind:value={formData.price_zone_2} oninput={() => isUserEditingZones = true} class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-100 outline-none text-sm font-bold text-gray-600" placeholder="0"/>
                                <span class="text-[9px] text-gray-400 font-medium mt-1 block">🚛 Luar Jawa (+10% Auto)</span>
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-gray-600 mb-1">Harga Zona 3</label>
                                <input type="number" bind:value={formData.price_zone_3} oninput={() => isUserEditingZones = true} class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-100 outline-none text-sm font-bold text-gray-600" placeholder="0"/>
                                <span class="text-[9px] text-gray-400 font-medium mt-1 block">✈️ Indo Timur (+25% Auto)</span>
                            </div>
                            
                            <div class="md:col-span-3">
                                <label class="block text-xs font-bold text-gray-600 mb-1">Stok Awal <span class="text-red-500">*</span></label>
                                <input type="number" bind:value={formData.stock} required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-100 outline-none text-sm" placeholder="0"/>
                            </div>
                            
                            <div class="md:col-span-3 bg-blue-50/50 p-3 rounded-lg border border-blue-100">
