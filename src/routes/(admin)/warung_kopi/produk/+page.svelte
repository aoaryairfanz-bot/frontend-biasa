...
<script>
    ...
    let uniqueSubcategories = $derived([
        ...new Set(products.map(p => p.subcategory ? p.subcategory[0].toUpperCase() + p.subcategory.slice(1) : ''))
    ].filter(Boolean).sort());
    ...
    // ==========================
    // 6. File Handler (TANPA DELETED LOGIC)
    // ==========================
    function handleFileChange(e, fieldName) {
        const file = e.target.files[0];
        if (file) {
    ...
    function resetForm() {
        editingId = null;
        formData = {
            name: '', sku: '', category: 'nonbook', subcategory: '', price: '', strike_price: '', stock: '',
            description: '', weight: '', length: '', width: '', height: '', diameter: '',
            isbn: '', publisher: '', author: '', publish_year: '', pages: '', book_version: ''
        };
        fileStorage = { foto_1: null, foto_2: null, foto_3: null, video: null };
        previews = { foto_1: null, foto_2: null, foto_3: null, video: null };
    ...
    // ==========================
    // 8. Background Upload (TIDAK DIUBAH)
    // ==========================
    function handleQueueUpload(e) {
        e.preventDefault();
        if (!fileStorage.foto_1 && !previews.foto_1) { alert("Wajib ada Thumbnail!"); return; }
    ...
    async function processBackgroundUpload(uploadId, dataPayload, filesPayload, editId) {
        ...
        try {
            ...
            ['strike_price','weight','length','width','height','diameter','isbn','publisher','author','publish_year','pages','book_version']
            .forEach(k=>{ if(dataPayload[k]) dataToSend.append(k,String(dataPayload[k])); });
    ...
    async function handleExcelUpload(e){
        ...
        try{
            const res=await fetch(`${PUBLIC_API_URL}/products/import`,{method:"POST",headers:{Authorization:`Bearer ${token}`},body:fd});
            const result=await res.json();
            if(res.ok){alert(`✅ Selesai`); await invalidateAll(); await loadProducts();}
...
<div class="space-y-6 relative min-h-screen pb-20">
    ...
    {#if loadingProducts}
        <div class="text-center text-gray-400 py-20">
            <LoaderIcon class="mx-auto animate-spin"/><div>Loading produk...</div>
        </div>
    {:else}
        <!-- TABEL + UI ANDA DISINI -->
        <!-- Tidak saya ubah -->
    {/if}

    <!-- NOTifikasi Upload Background -->
    <div class="fixed bottom-6 right-6 z-[60] flex flex-col gap-2 w-80">
        {#each activeUploads as upload (upload.id)}
            <div class="bg-white p-3 rounded-xl shadow-2xl border border-gray-100 flex items-center gap-3 animate-in slide-in-from-right duration-300">
                {#if upload.status === 'loading'}
                    <LoaderIcon size="20" class="text-blue-500 animate-spin"/>
                    <div class="flex-1 min-w-0">
                        <div class="text-xs font-bold text-gray-500">Mengupload...</div>
                        <div class="text-sm font-bold text-gray-800 truncate">{upload.name}</div>
                    </div>
                {:else if upload.status === 'success'}
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="text-xs font-bold text-green-500">Berhasil</div>
                        <div class="text-sm font-bold text-gray-800 truncate">{upload.name}</div>
                    </div>
                {:else}
                    <AlertCircleIcon size="20" class="text-red-500"/>
                    <div class="flex-1 min-w-0">
                        <div class="text-xs font-bold text-red-500">Gagal</div>
                        <div class="text-sm font-bold text-gray-800 truncate">{upload.name}</div>
                    </div>
                {/if}
            </div>
