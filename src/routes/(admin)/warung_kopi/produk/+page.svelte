...
<script>
    ...
    import { invalidateAll } from '$app/navigation'; 
    import { onMount } from 'svelte'; // Tambahkan onMount
    import { PUBLIC_API_URL } from '$env/static/public';
    // Import browser-image-compression TETAP ADA untuk logic (tapi nanti kita bypass jika mau)
    // Jika Baginda benar-benar ingin upload file ASLI tanpa kompresi sama sekali, kita skip penggunaannya.
    
    // --- 1. STATE HALAMAN ---
    let products = $state([]); // Data di-fetch client-side
    let isLoadingData = $state(true); // Loading indicator awal
    let searchQuery = $state('');
    let activeCategory = $state('all'); 
    let viewMode = $state('list'); 
    let currentPage = $state(1);
    ...
    // --- 2. FETCH DATA (CLIENT SIDE - INSTANT LOAD) ---
    onMount(async () => {
        const token = localStorage.getItem("token");
        if (!token) return; // Atau redirect ke login
        ...
        try {
            ...
            if (res.ok) {
                ...
                // Handle struktur data yg mungkin berbeda
    ...
    // --- 5. LOGIKA FILTER (Update: Search SKU) ---
    let filteredProducts = $derived(products.filter(p => {
        const term = searchQuery.toLowerCase();
        // Cari di Nama, SKU, atau Subkategori
    ...
    function removeFile(fieldName) {
        fileStorage[fieldName] = null;
        previews[fieldName] = null;
        const input = document.getElementById(fieldName); // Fix ID selector
        if(input) input.value = '';
    }
    ...
    // Fungsi Worker (Upload ASLI tanpa kompresi)
    async function processBackgroundUpload(uploadId, dataPayload, filesPayload, editId) {
        const token = localStorage.getItem("token");
        ...
        try {
            ...
            // UPLOAD FILE ASLI (Tanpa imageCompression)
            const imageFields = ['foto_1', 'foto_2', 'foto_3'];
            for (const field of imageFields) {
                if (filesPayload[field] && filesPayload[field] instanceof File) {
                    // LANGSUNG APPEND FILE ASLI
                    dataToSend.append(field, filesPayload[field]); 
                }
            }
            if (filesPayload.video instanceof File) dataToSend.append('video', filesPayload.video);
            ...
            if (res.ok) {
                updateUploadStatus(uploadId, 'success');
                // Manual Refresh Data (karena invalidateAll butuh page load)
                // Kita fetch ulang manual di background
                const refreshRes = await fetch(`${PUBLIC_API_URL}/products/`, { headers: { "Authorization": `Bearer ${token}` }});
                if(refreshRes.ok) {
                    const newData = await refreshRes.json();
                    if(Array.isArray(newData)) products = newData;
                    else if(newData.products) products = newData.products;
                    else if(newData.data) products = newData.data;
                }
            } else {
                const err = await res.json();
                updateUploadStatus(uploadId, 'error', err.detail || 'Gagal');
        ...
        <div class="p-8 max-h-[80vh] overflow-y-auto custom-scrollbar">
            <form class="grid grid-cols-1 md:grid-cols-12 gap-8" on:submit={handleQueueUpload}>
                ...
                <div class="md:col-span-4 space-y-6">
                    ...
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-1">Thumbnail</label>
                        <div class="relative border-2 border-dashed {fileStorage.foto_1 ? 'border-green-400' : 'border-blue-300'} rounded-xl h-40 flex items-center justify-center bg-gray-50 hover:bg-gray-100 transition">
                            {#if previews.foto_1}
                                <img src={previews.foto_1} alt="Preview" class="h-full w-full object-cover rounded-lg" />
                                <button type="button" on:click={() => removeFile('foto_1')} class="absolute top-2 right-2 bg-red-500 text-white rounded-full p-1"><XIcon size="14"/></button>
                            {:else}
                                <div class="text-center"><UploadCloudIcon size="24" class="mx-auto text-gray-400"/><span class="text-xs font-bold text-gray-500">Pilih Foto</span></div>
                            {/if}
                            <input type="file" id="foto_1" accept="image/*" on:change={(e) => handleFileChange(e, 'foto_1')} class="absolute inset-0 opacity-0 cursor-pointer" />
                        </div>
                    </div>
                    <div class="grid grid-cols-2 gap-2">
                        {#each ['foto_2', 'foto_3'] as f}
                        <div class="relative border-2 border-dashed rounded-xl h-24 flex items-center justify-center bg-gray-50 hover:bg-gray-100 transition">
                            {#if previews[f]} <img src={previews[f]} alt="Preview" class="h-full w-full object-cover rounded-lg" /> <button type="button" on:click={() => removeFile(f)} class="absolute top-0 right-0 bg-red-500 text-white rounded-full p-1"><XIcon size="10"/></button>
                            {:else} <PlusIcon size="20" class="text-gray-400"/> {/if}
                            <input type="file" id={f} accept="image/*" on:change={(e) => handleFileChange(e, f)} class="absolute inset-0 opacity-0 cursor-pointer" />
                        </div>
...
<script>
    ...
    async function refreshDataInBackground() {
        ...
        try {
            ...
            console.error("Gagal sync server", e);
        } finally {
            isLoadingData = false;
        }
    }
...
<div class="space-y-6 relative min-h-screen pb-20">
    ...
    {:else}
        {#if viewMode === 'grid'}
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                {#each paginatedProducts as product}
                <div class="bg-white rounded-2xl shadow-sm hover:shadow-lg transition-shadow duration-300 overflow-hidden group flex flex-col">
                    <div class="relative aspect-square bg-gray-50 overflow-hidden">
                        <img src={product.image_1_url || 'https://placehold.co/300?text=No+Img'} alt={product.name} class="w-full h-full object-cover" loading="lazy" />
