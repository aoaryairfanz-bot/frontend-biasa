<!-- <script>
    ...
    // --- 1. STATE HALAMAN ---
    let products = $state([]); 
    let discounts = $state([]); // [BARU] Simpan daftar voucher
    ...
    const CACHE_KEY = 'admin_products_cache_v3'; // [UPDATE] Versi cache baru

    // --- 2. FETCH DATA (PRODUK & DISKON) ---
    onMount(async () => {
        const token = localStorage.getItem("token");
        if (!token) { goto('/login'); return; }

        // A. Load Cache Dulu (Biar Cepat)
        try {
        ...
        // B. Sync Data Terbaru (Produk & Voucher)
    ...
    // [BARU] Ambil Data Voucher untuk Dropdown
    ...
    // [UPDATE] Tambah discount_id di form
    let formData = $state({
        name: '', sku: '', category: 'nonbook', subcategory: '', 
        price: '', strike_price: '', stock: '', description: '',
        discount_id: 0, // [BARU] Menyimpan ID Voucher yg dipilih (0 = Tidak ada)
    ...
    function openEditModal(product) {
        ...
        formData = { 
            ...
            // [BARU] Ambil ID diskon jika ada, kalau null set 0
    ...
    async function processBackgroundUpload(uploadId, dataPayload, filesPayload, editId) {
        const token = localStorage.getItem("token");
        try {
            // LANGKAH 1: Simpan Data Produk (Nama, Harga, Foto, dll)
            ...
            if (res.ok) {
                ...
                // LANGKAH 2: Simpan Data Diskon (Assign Discount)
                // Kita panggil endpoint terpisah agar aman
                ...
                refreshDataInBackground(); // Refresh tabel agar harga diskon muncul
            } else {
...
<script>
    ...
    const CACHE_KEY = 'admin_products_cache_v3'; // [UPDATE] Cache Version
    ...
    // [UPDATE] Tambah discount_id
    let formData = $state({
        name: '', sku: '', category: 'nonbook', subcategory: '', 
        price: '', strike_price: '', stock: '', description: '',
        discount_id: 0, // [BARU] ID Voucher (0 = None)
        weight: '', length: '', width: '', height: '', diameter: '',
        isbn: '', publisher: '', author: '', publish_year: '', pages: '', book_version: ''
    });
    ...
    function openEditModal(product) {
        ...
        formData = { 
            ...
            name: toTitleCase(product.name), 
            subcategory: toTitleCase(product.subcategory),
            // [BARU] Set discount_id dari data produk (jika ada)
            discount_id: product.discount ? product.discount.id : 0
        };
        previews = { 
            foto_1: product.image_1_url, foto_2: product.image_2_url, 
    ...
    // --- 7. UPLOAD & SAVE (DENGAN ASSIGN DISCOUNT) ---
    function handleQueueUpload(e) {
        e.preventDefault();
        const currentData = { ...formData }; 
        const currentFiles = { ...fileStorage };
    ...
    async function processBackgroundUpload(uploadId, dataPayload, filesPayload, editId) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                // [BARU] STEP 2: Assign Discount (Terpisah agar aman)
                const discFormData = new FormData();
                discFormData.append('discount_id', dataPayload.discount_id || 0);
